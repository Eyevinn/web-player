...
export default class EyevinnSkin {
  ...
  setupEventListeners() {
    this.player.on(PlayerEvent.TIME_UPDATE, ({ currentTime, duration }) =>
      this.progressbar.update(currentTime, duration)
    );
    this.player.on(PlayerEvent.PLAY, () => this.playPauseButton.update(true));
    this.player.on(PlayerEvent.PAUSE, () => this.playPauseButton.update(false));
  }
}
