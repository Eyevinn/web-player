...
async function main() {
  ...
  const vmapUrlInput = document.querySelector('#vmap-input');
  ...
  async function load() {
    try {
      ...
      const ads = {
        displayAds: adsCheckbox.checked,
        vmapUrl: vmapUrlInput.value
      };
      await player.load(manifestInput.value, autoplayCheckbox.checked, ads);
